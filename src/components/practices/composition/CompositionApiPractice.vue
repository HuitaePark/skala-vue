...
const apiRows = [
  { category: 'Application', functions: 'createApp, createSSRApp, app.mount()', description: 'Vue 애플리케이션을 만들고 DOM에 연결합니다.' },
  { category: 'Reactive State', functions: 'ref, reactive, readonly, shallowRef', description: '반응형 상태를 선언하고 변경을 추적합니다.' },
  { category: 'Computed & Watchers', functions: 'computed, watch, watchEffect', description: '계산값과 데이터 변경 감시를 구성합니다.' },
  { category: 'Lifecycle Hooks', functions: 'setup, onMounted, onUpdated, onUnmounted', description: '컴포넌트 생명주기 시점에 코드를 실행합니다.' },
  { category: 'Rendering', functions: 'h, resolveComponent, nextTick', description: '렌더링과 DOM 업데이트 타이밍을 제어합니다.' },
  { category: 'Dependency Injection', functions: 'provide, inject', description: '컴포넌트 트리 전체에 값을 전달합니다.' },
]
...
<template>
  <div class="composition-practice-stack">
    ...
    <section class="practice-section composition-overview">
      ...
      <p>
        Vue 3의 <code>&lt;script setup&gt;</code>에서 상태와 로직을 기능별로 모아 작성합니다.
        아래 표는 PDF에 나온 Application, Reactive State, Computed &amp; Watchers의 주요 함수를
        실행 가능한 예제와 함께 정리한 것입니다.
      </p>
...
createApp(App).mount('#app')</code></pre>
      ...
      <div class="composition-api-table" role="region" aria-label="Composition API 함수 요약">
        <div class="composition-api-row composition-api-head"><span>분류</span><span>함수</span><span>역할</span></div>
    ...
    <section class="practice-section reactive-practice">
      ...
      <p>
        <code>ref()</code>는 원시값·배열·객체를 하나의 반응형 참조로 감싸고,
        <code>reactive()</code>는 객체와 배열의 속성을 직접 변경하는 방식입니다.
      </p>
...
items.push('과일 3')</code></pre>
      ...
      <div class="composition-demo-grid">
        <div class="composition-demo-card">
          <h3>1) ref() 예제</h3>
          <p>Ref 카운트: <strong>{{ refCount }}</strong></p>
          <label class="composition-field">
            <span>이름</span>
            <input v-model="refName" type="text" />
          </label>
          <p>입력값: <strong>{{ refName }}</strong></p>
          <p>활성 상태: <strong>{{ refIsActive ? '활성' : '비활성' }}</strong></p>
          <p>과일 목록: <strong>{{ refItems.join(', ') }}</strong></p>
          <p>사용자 정보: <strong>{{ refUser.name }} · {{ refUser.age }}세</strong></p>
        ...
        <div class="composition-demo-card">
          <h3>2) reactive() 예제</h3>
          <p>이름: <strong>{{ reactiveUser.name }}</strong> / 나이: <strong>{{ reactiveUser.age }}세</strong></p>
    ...
    <section class="practice-section computed-practice">
      ...
      <p>
        일반 함수는 컴포넌트가 다시 렌더링될 때마다 실행되지만, <code>computed()</code>는
        의존하는 값이 바뀔 때만 다시 계산하고 그 사이에는 이전 결과를 재사용합니다.
      </p>
...
// count가 아닌 dummy만 바뀌면 computed 결과는 캐시됩니다.</code></pre>
      ...
      <div class="computed-demo-card">
        <p>count: <strong>{{ computedCount }}</strong> · dummy: <strong>{{ dummyCount }}</strong></p>
        <div class="composition-button-row">
        ...
        <div class="computed-result-grid">
          <p>일반 함수 결과: <strong>{{ getMethodResult() }}</strong></p>
          <p>Computed 결과: <strong>{{ computedDouble }}</strong></p>
          <p>함수 실행 횟수: <strong>{{ methodRunCount }}</strong> · computed 재계산 횟수: <strong>{{ computedRunCount }}</strong></p>
        </div>
      </div>
    </section>
    ...
    <section class="practice-section watchers-practice">
      ...
      <p v-if="props.challengeOnly">
        <code>watch()</code>는 지정한 반응형 source의 이전 값과 현재 값을 전달합니다.
        단일 값, 여러 값, 중첩 객체와 reactive 객체를 각각 감시해 보세요.
      </p>
      <p v-else>
        <code>watch()</code>는 지정한 반응형 source의 이전 값과 현재 값을 전달합니다.
        단일 값, 여러 값, 중첩 객체와 배열을 각각 감시해 보세요.
      </p>
...
watch(reactiveUser, () =&gt; { ... })</code></pre>
      ...
      <div class="watch-demo-grid">
        <div class="watch-demo-card">
          ...
          <p>count: <strong>{{ watchedCount }}</strong></p>
        ...
        <div class="watch-demo-card">
          ...
          <label class="composition-field"><span>도시</span><input v-model="deepProfile.address.city" type="text" /></label>
        ...
        <div class="watch-demo-card">
          ...
          <p>이름: <strong>{{ reactiveWatchUser.name }}</strong> / 나이: <strong>{{ reactiveWatchUser.age }}세</strong></p>
    ...
    <section class="practice-section effect-practice">
      ...
      <p>
        감시할 source를 따로 적지 않아도 effect 내부에서 읽은 반응형 값을 자동으로 추적합니다.
        컴포넌트가 처음 열릴 때도 한 번 실행되는 특징을 확인하세요.
      </p>
...
watchEffect(() =&gt; {
  logMessage.value = `[자동 감지] 이름: ${username.value} / 나이: ${age.value}세`
      ...
      <div class="effect-demo-card">
        <p>이름: <strong>{{ effectUsername }}</strong> / 나이: <strong>{{ effectAge }}세</strong></p>
        <div class="composition-button-row">
      ...
      <div class="watcher-reference-table" role="region" aria-label="Computed와 Watcher 함수 요약">
        <div class="watcher-reference-row watcher-reference-head"><span>분류</span><span>함수</span><span>설명</span></div>
        <div class="watcher-reference-row"><span>계산된 속성 선언</span><code>computed</code><span>반응형 상태에서 파생 값을 계산하고 캐시합니다.</span></div>
        <div class="watcher-reference-row"><span>지정형 감시자</span><code>watch</code><span>특정 source의 변경과 이전·현재 값을 감시합니다.</span></div>
        <div class="watcher-reference-row"><span>자동 의존성 감시자</span><code>watchEffect</code><span>effect 내부에서 읽은 반응형 값을 자동 추적합니다.</span></div>
        <div class="watcher-reference-row"><span>실행 시점 제어</span><code>watchPostEffect / watchSyncEffect</code><span>DOM 반영 후 또는 동기적으로 effect를 실행합니다.</span></div>
      </div>
    </section>
    ...
    <section class="practice-section composition-challenge">
      ...
      <p v-else>PDF 144쪽의 과제 범위인 Reactive State와 Computed &amp; Watchers 예제를 이 페이지에서 모두 실행했습니다.</p>
